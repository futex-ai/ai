...
(`64:ff9b::/96`), and 6to4 transition ranges are rejected. Metadata marked
`no-store` or `no-cache`, or carrying an invalid `max-age`, is never reused.
The crate does not provide a browser, callback listener, Keychain/database
implementation, or product UI.
