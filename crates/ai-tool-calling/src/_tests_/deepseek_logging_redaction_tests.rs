//! DeepSeek model-call logging redaction regression tests.

use std::sync::Arc;

use ai_interface::{
    ConversationMessage, ConversationRole, DeepSeekToolCallContext, FinishReason, Logger,
    LoggerMock, Model, ModelCallLogEntry, ModelCallLogResult, ModelError, ModelMock, ModelRequest,
...
#[tokio::test]
async fn deepseek_reasoning_is_replayed_but_redacted_from_success_logs() {
    let provider_context = deepseek_context();
    ...
    assert!(runtime.conversation().iter().any(|message| {
        message.role == ConversationRole::Assistant
            && contains_deepseek_context(&message.provider_context)
    }));
}

#[tokio::test]
async fn deepseek_reasoning_is_redacted_from_failed_request_logs() {
    let provider_context = deepseek_context();
    let replay_context = provider_context.clone();
    let model: Arc<dyn Model> = Arc::new(Unimock::new(
        ModelMock::complete
...
fn deepseek_context() -> Vec<ProviderConversationItem> {
    vec![ProviderConversationItem::DeepSeekAssistantMessage {
        content: "calling tool".to_owned(),
        reasoning_content: Some("private DeepSeek reasoning".to_owned()),
...
fn tool_response(provider_context: Vec<ProviderConversationItem>) -> ModelResponse {
    ModelResponse {
        ...
        catalog_model_id: Some("deepseek-v4-pro".to_owned()),
...
fn assert_log_entry_is_redacted(entry: &ModelCallLogEntry) {
    ...
    assert!(!contains_deepseek_context(&response.provider_context));
}
...
fn assert_request_is_redacted(request: &ModelRequest) {
    assert!(
        request
            .messages
            .iter()
            .all(|message| !contains_deepseek_context(&message.provider_context))
    );
}

fn contains_deepseek_context(context: &[ProviderConversationItem]) -> bool {
