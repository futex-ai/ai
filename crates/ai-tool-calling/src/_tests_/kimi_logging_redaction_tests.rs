//! Kimi model-call logging redaction regression tests.

use std::sync::Arc;

use ai_interface::{
    ConversationMessage, ConversationRole, FinishReason, KimiToolCallContext, Logger, LoggerMock,
    Model, ModelCallLogEntry, ModelCallLogResult, ModelError, ModelMock, ModelRequest,
    ModelResponse, ModelUsage, ProviderConversationItem, ToolCall,
};
use serde_json::json;
...
#[tokio::test]
async fn kimi_reasoning_is_replayed_but_redacted_from_success_logs() {
    let provider_context = kimi_context();
    ...
    assert!(runtime.conversation().iter().any(|message| {
        message.role == ConversationRole::Assistant
            && contains_kimi_context(&message.provider_context)
    }));
}

#[tokio::test]
async fn kimi_reasoning_is_redacted_from_failed_request_logs() {
    let provider_context = kimi_context();
    let replay_context = provider_context.clone();
    let model: Arc<dyn Model> = Arc::new(Unimock::new(
        ModelMock::complete
...
fn kimi_context() -> Vec<ProviderConversationItem> {
    vec![ProviderConversationItem::KimiAssistantMessage {
        content: Some("calling tool".to_owned()),
        reasoning_content: Some("private Kimi reasoning".to_owned()),
...
fn assert_log_entry_is_redacted(entry: &ModelCallLogEntry) {
    ...
    assert!(!contains_kimi_context(&response.provider_context));
}
...
fn assert_request_is_redacted(request: &ModelRequest) {
    assert!(
        request
            .messages
            .iter()
            .all(|message| !contains_kimi_context(&message.provider_context))
    );
}

fn contains_kimi_context(context: &[ProviderConversationItem]) -> bool {
    context
        .iter()
        .any(|item| matches!(item, ProviderConversationItem::KimiAssistantMessage { .. }))
}
