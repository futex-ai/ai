//! Qwen model-call logging redaction regression tests.

use std::sync::Arc;

use ai_interface::{
    ConversationMessage, ConversationRole, FinishReason, Logger, LoggerMock, Model,
    ModelCallLogEntry, ModelCallLogResult, ModelError, ModelMock, ModelRequest, ModelResponse,
    ModelUsage, ProviderConversationItem, QwenToolCallContext, ToolCall,
};
use serde_json::json;
...
#[tokio::test]
async fn qwen_reasoning_is_replayed_but_redacted_from_success_logs() {
    let provider_context = qwen_context();
    ...
    assert!(runtime.conversation().iter().any(|message| {
        message.role == ConversationRole::Assistant
            && contains_qwen_context(&message.provider_context)
    }));
}

#[tokio::test]
async fn qwen_reasoning_is_redacted_from_failed_request_logs() {
    let provider_context = qwen_context();
    let replay_context = provider_context.clone();
    let model: Arc<dyn Model> = Arc::new(Unimock::new(
        ModelMock::complete
...
fn qwen_context() -> Vec<ProviderConversationItem> {
    vec![ProviderConversationItem::QwenAssistantMessage {
        content: Some("calling tool".to_owned()),
        reasoning_content: Some("private Qwen reasoning".to_owned()),
...
fn tool_response(provider_context: Vec<ProviderConversationItem>) -> ModelResponse {
    ModelResponse {
        ...
        model_id: "qwen3.7-plus".to_owned(),
        catalog_model_id: Some("qwen3.7-plus".to_owned()),
...
fn assert_log_entry_is_redacted(entry: &ModelCallLogEntry) {
    ...
    assert!(!contains_qwen_context(&response.provider_context));
}
...
fn assert_request_is_redacted(request: &ModelRequest) {
    assert!(
        request
            .messages
            .iter()
            .all(|message| !contains_qwen_context(&message.provider_context))
    );
}

fn contains_qwen_context(context: &[ProviderConversationItem]) -> bool {
    context
        .iter()
        .any(|item| matches!(item, ProviderConversationItem::QwenAssistantMessage { .. }))
}
