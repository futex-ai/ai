...
| MiniMax | M3 incremental fragments; one validated terminal M2.x inferred value | Append-only reasoning fragments when exposed | Existing shared accumulation and validated EOF rules |
...
MiniMax emits M3 assistant content incrementally. M2.x may send snapshots or
fragments, so the adapter infers the shape per stream while withholding
assistant content. A nonempty value that strictly extends retained text
replaces it, and extending a nonempty retained prefix records snapshot
evidence. Other nonempty values append until that evidence exists and become
replacing snapshots afterward; empty values are ignored. After terminal
validation, the adapter emits the inferred nonempty text once. Append-only
