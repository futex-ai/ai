...
| MiniMax | Same opt-in; final complete usage, then `[DONE]` or clean EOF | Accumulate M3 content deltas directly. For each M2.x stream, withhold content and infer its shape: a value strictly extending retained text replaces it and records snapshot evidence when the prefix was nonempty, other nonempty values append until that evidence and replace afterward, and empty values are ignored. Restore the result after terminal validation and emit it as one terminal assistant event. Retain the last nonempty `reasoning_details` snapshot as canonical because later snapshots may revise earlier text; accept EOF only after choices, finish reason, usage, and tool metadata validate |
