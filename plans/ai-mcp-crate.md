...
- [ ] Add `crates/ai-mcp` to the workspace members and add
      ...
      dependencies.
- [ ] Create the crate manifest with workspace package metadata, the
...
- [ ] Keep `lib.rs` as a thin documented module root, add
      ...
      contracts.
- [ ] Add `McpServerConfig`, its documented defaults, and validation for the
      `[a-z0-9_-]{1,32}` server-key contract without introducing product or
      credential state.
- [ ] Define the typed `ai_mcp::{Error, Result}` contract, including actionable
...
- [ ] Run `git add -A`, commit the green protocol-client milestone with a
