...
- [ ] Add failing serde tests for inline envelopes without output ids, window
...
- [ ] Add the typed `remainder_unavailable` reason enum with the
...
- [ ] Write the reader's tool description per the protocol's model guidance:
...
- [ ] Add failing regression tests for a small success, an exact
...
- [ ] Serialize and measure each successful raw output once; return inline
      envelopes without touching the store, and store only outputs above the
      inline limit before constructing their first window.
- [ ] Assert in the small-success test that inline outputs are never written
      to the store and never consume aggregate budget.
- [ ] Degrade instead of dropping when a successful output cannot be stored:
...
- [ ] Keep typed runtime errors for policy validation and intrinsic-read
      failures; `run()` continues after recoverable read failures.
- [ ] Make budget-exhaustion tests order-agnostic: assert that some sibling
      call degrades without pinning which one.
- [ ] Change `ToolExecutionRecord` to expose an optional `output_id`,
...
- [ ] Document that redaction must be applied to raw output before the store
      write and envelope construction, because window content is an opaque
      byte range that cannot be redacted afterward.
- [ ] Document mixed-format history: conversations persisted before adoption
...
- [ ] Document that hosts should avoid suspending a run mid-pagination, may
