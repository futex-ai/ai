...
fn split_integration_target_uses_a_directory_root() {
    ...
    let legacy_root = manifest_dir.join("tests/live_judgments.rs");
    let target_root = manifest_dir.join("tests/live_judgments/mod.rs");
    ...
    assert!(
        XTASK_MANIFEST.contains(
            "[[test]]\nname = \"live_judgments\"\npath = \"tests/live_judgments/mod.rs\""
        ),
    ...
    for entry in fs::read_dir(manifest_dir.join("tests/live_judgments"))
